...
public class NetworkAlertsManager {
    ...
    public static Supplier<StatusCode> addStatusCodeAlert(String groupName, String message, Supplier<StatusCode> statusCode) {
        addWarningAlert(groupName, () -> message + statusCode.get().getDescription(),
            () -> statusCode.get().isWarning());
        addErrorAlert(groupName, () -> message + statusCode.get().getDescription(), 
            () -> statusCode.get().isError());
        return statusCode;
    }

    // public static Supplier<StatusCode> addStatusCodeAlert(String message, LongSupplier statusCodeValue) {
    //     Supplier<StatusCode> statusCode = () -> StatusCode.valueOf((int)statusCodeValue.getAsLong());
    //     return addStatusCodeAlert(message, statusCode);
    // }

    // public static Supplier<StatusCode> addStatusCodeAlert(String groupName, String message, LongSupplier statusCodeValue) {
    //     Supplier<StatusCode> statusCode = () -> StatusCode.valueOf((int)statusCodeValue.getAsLong());
    //     return addStatusCodeAlert(groupName, message, statusCode);
    // }
    ...
    public static void addSparkMotorAlert(String groupName, String message, Supplier<Faults> faults, Supplier<Warnings> warnings) {
        addErrorAlert(groupName, () -> message + getSparkFaultsMessage(faults.get()),
            () -> faults.get().rawBits != 0);
        addWarningAlert(groupName, () -> message + getSprakWarningsMessage(warnings.get()),
            () -> warnings.get().rawBits != 0);
    }
